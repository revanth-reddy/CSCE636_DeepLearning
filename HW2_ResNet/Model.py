...
class Cifar(object):
    ...
    def setup(self, training):
        ...
        if training:
            ...
            ### YOUR CODE HERE
            # cross entropy
            logLoss = tf.losses.sparse_softmax_cross_entropy(
                              labels=self.labels, logits=logits)


            # final loss function
            self.losses =  logLoss + l2_loss
    ...
    def train(self, x_train, y_train, max_epoch):
        ...
        for epoch in range(1, max_epoch+1):
            ...
            for i in range(num_batches):
                ...
                bs_low = self.conf.batch_size*i
                bs_high = self.conf.batch_size*(i+1)
                train_batch = curr_x_train[bs_low:bs_high]
                y_batch = curr_y_train[bs_low:bs_high]

                # Don't forget to use "parse_record" to perform data preprocessing.
                # Replace hard coded shape with input shape
                x_batch = np.zeros((np.shape(train_batch)[0],32,32,3))
                for row in range(np.shape(train_batch)[0]):
                    x_batch[row] = parse_record(train_batch[row], training=True)
    ...
    def test_or_validate(self, x, y, checkpoint_num_list):
        ...
        for checkpoint_num in checkpoint_num_list:
            ...
            for i in tqdm(range(x.shape[0])):
                ### YOUR CODE HERE
                # Preprocess data using parse_record
                x_test = parse_record(x[i], training=False).reshape(1, 32, 32, 3)

                feed_dict = {self.inputs: x_test,
                            self.labels: y[i]}
                preds.append(self.sess.run(self.preds, feed_dict))
                ### END CODE HERE
